...
- Use `body-01` for editorial paragraphs in section patterns unless the approved pattern explicitly specifies another scale. `FullWidthFeatureSection` uses `body-02` from its Figma source. More compact styles are reserved for the opening hero, list items, captions, legal copy and other explicitly dense UI contexts.
