...
- All full-page examples use the shared `ExampleSiteChrome` template. Global Navbar and Footer content lives only in `site-shell.md`; never source it from Landing page or duplicate it in another page fixture. Every example Markdown file owns one `type: page` record with `navbarTheme`, `navbarBackground`, `footerTheme` and `footerBackground`. Landing page follows the same contract as every subpage.
...
- The Labbet example reads its page-shell settings plus ordered Hero, Feature and Icon List records from `labbet-page.md` and composes global Navbar and Footer content from `site-shell.md`. Keep Labbet as the current main-navigation item and preserve the page's single `h1` in the split Hero.
...
- Register reusable editorial images under stable semantic keys in the shared `demoImages` registry instead of exposing source export filenames. Show each newly registered resource in the Components Image catalog with its exact Markdown key, and keep intrinsic dimensions and alt text synchronized there.
- Image-section Markdown/CMS records use `type: image | video` and `variant: grid | full-width | full-width-scroll`; the mapping layer translates `variant` to `ImageSection`'s internal `layout` prop. Image records reference the image registry. Video records reference the video registry and require `playback: background | controls`. The registry supports native files and Vimeo resources identified by stable video ID plus optional privacy hash; Vimeo resources also record intrinsic width and height so embedded background playback can be cropped like `object-fit: cover`. Keep the Vimeo iframe's layout viewport at those intrinsic dimensions and apply the responsive cover factor as a compositor transform; never resize a portrait iframe to the full calculated cover height on every frame. Pause the Vimeo player outside a small viewport margin. Retain the supplied review URL only as provenance and never pass it to a native `<video>` element. Background playback is autoplaying, looped, muted, control-free and removed from keyboard focus; pause native video or replace a Vimeo embed with its poster for `prefers-reduced-motion`. Verify background video in both `full-width` and `full-width-scroll` specimens. Controlled playback starts paused and unmuted with the provider's play, pause and volume controls; provide captions in the video asset or provider whenever speech or informative audio exists. New records set both `backgroundTop` and `backgroundBottom`; the first covers one third and the second two thirds using flat semantic surfaces. Optional `backgroundTopTheme` and `backgroundBottomTheme` let a section bridge local Light and Dark modes. Keep `background` as the monochrome fallback for existing content, never implement the split as a gradient, and expose both background selectors in every Image Section specimen. `full-width-scroll` owns a responsive cropped frame capped at 800 px and vertically pans its overscanned medium with scroll; disable that motion for `prefers-reduced-motion`. Keep the legacy `layout` read fallback only for existing content and do not document it for editors.
