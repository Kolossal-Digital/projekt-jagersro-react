...
- Image-section Markdown/CMS records use `variant: grid | full-width | full-width-scroll`; the mapping layer translates it to `ImageSection`'s internal `layout` prop. New records set both `backgroundTop` and `backgroundBottom`; the first covers one third and the second two thirds using flat semantic surfaces. Optional `backgroundTopTheme` and `backgroundBottomTheme` let a section bridge local Light and Dark modes. Keep `background` as the monochrome fallback for existing content, never implement the split as a gradient, and expose both background selectors in every Image Section specimen. `full-width-scroll` owns a responsive cropped frame capped at 800 px and vertically pans its overscanned image with scroll; disable that motion for `prefers-reduced-motion`. Keep the legacy `layout` read fallback only for existing content and do not document it for editors.
